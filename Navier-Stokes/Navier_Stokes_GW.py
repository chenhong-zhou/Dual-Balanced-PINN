...
for i in range(4): 
    ...
    for j in range(1):
        ...
        for epoch in range(Adam_n_epochs):     
            ...
            l_bc  = torch.mean((xlb_pred)**2)
            l_bc += torch.mean((xrb_pred)**2)
            ...
            l_bc += torch.mean((ylb_pred)**2)
            l_bc += torch.mean((yrb_pred - U_Y_rb)**2)

            L_t = torch.stack((l_reg, l_bc))
            ...
            if epoch%i_print==0: 
                ...
                print("Adam optimizing:  method={}, j={}, epoch {}/{}, loss={:.6f}, loss_bc={:.6f},loss_r={:.6f}, lambda={:.4f}, lr={:,.7f}, L2 error (%)={:.6f}".format(method, j, epoch+1, Adam_n_epochs, loss.item(), l_bc.item(), l_reg.item(), lambd.item(), optimizer.param_groups[0]['lr'], tmp*100)) 
